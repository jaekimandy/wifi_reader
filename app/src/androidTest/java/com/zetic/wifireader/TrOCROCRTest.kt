...
import com.zetic.wifireader.ocr.TrOCROCREngine
...
class TrOCROCRTest {
    ...
    private lateinit var trocrEngine: TrOCROCREngine

    @Before
    fun setup() {
        context = InstrumentationRegistry.getInstrumentation().targetContext
        trocrEngine = TrOCROCREngine(context)
    }
    ...
    @Test
    fun testTrOCRInitialization() = runBlocking {
        // Test if TrOCR can initialize with downloaded models
        val initialized = trocrEngine.initialize()

        assertTrue("TrOCR should initialize successfully", initialized)
    }
    ...
    fun testSimpleTextRecognition() = runBlocking {
        // Initialize TrOCR
        val initialized = trocrEngine.initialize()
        assertTrue("TrOCR should initialize", initialized)
        ...
        val textRegions = trocrEngine.extractText(bitmap)
        ...
        // Check if text is detected (TrOCR returns placeholder for now)
        assertTrue(
    ...
    fun testWiFiCredentialText() = runBlocking {
        ...
        // Check that TrOCR is working (placeholder text)
    ...
    fun testBoundingBoxExtraction() = runBlocking {
        ...
        val textRegions = trocrEngine.extractText(bitmap, boundingBox)
    ...
    fun testPerformance() = runBlocking {
        ...
        repeat(3) {
            val textRegions = trocrEngine.extractText(bitmap, null)
        ...
        println("3 TrOCR operations took: ${totalTime}ms")

        // Should complete within reasonable time (adjust based on device)
        assertTrue("TrOCR should complete within 15 seconds", totalTime < 15000)
    }

    @Test
    fun testModelLoading() = runBlocking {
        // Test that TrOCR models can be loaded successfully
        val initialized = trocrEngine.initialize()
        assertTrue("TrOCR models should load successfully", initialized)
